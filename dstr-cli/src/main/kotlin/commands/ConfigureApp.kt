...
class ConfigureApp : SuspendingCliktCommand(name = "configure") {
    ...
    override suspend fun run() {
        if (successUrl == null && logoutRedirectUrl == null && webhookSecret == null && sessionTtl == null) {
            throw IllegalArgumentException(
                "Provide at least one of --success-url, --logout-url, --webhook-secret, --session-ttl"
            )
        }
        val resp = client.patch("${DusterConfig.dusterBaseUrl}/duster/api/v1/internal/apps/config") {
            ...
            setBody(AppConfigUpdateRequest(successUrl, logoutRedirectUrl, webhookSecret, sessionTtl))
        }
